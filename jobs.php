...
<main class="page-container">
    ...
    <div style="background-color: var(--light-grey); padding: 15px; border-radius: 8px; margin-bottom: 25px;">
        <form method="get" action="jobs.php" style="display: flex; gap: 10px; max-width: 100%; margin: 0; padding: 0; background: transparent;">
            ...
            <button type="submit" style="padding: 10px 20px; background-color: var(--primary-blue); color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer;">Search Jobs</button>
            <?php if ($search_query !== ""): ?>
    ...
    <div style="width: 70%; float: left; box-sizing: border-box; padding-right: 20px;">
        <?php if ($result && mysqli_num_rows($result) > 0): ?>
            <?php while ($job = mysqli_fetch_assoc($result)): ?>
                <section style="margin-bottom: 40px; border-bottom: 1px solid #eee; padding-bottom: 20px;">
                    <h3 style="color: var(--primary-blue); margin-bottom: 5px; font-size: 1.4rem;"><?php echo htmlspecialchars($job['title']); ?></h3>
